  ...
  switch (address)
  {
    ...
    case MB_HOLD_COM_MB_TIMEOUT:
      *value = conf.com.mb_timeout;
      break;
    case MB_HOLD_PF_CUTOFF_LP_0:
      *value = *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_LP) + 0);
      break;
    case MB_HOLD_PF_CUTOFF_LP_1:
      *value = *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_LP) + 1);
      break;
    case MB_HOLD_PF_CUTOFF_HP_0:
      *value = *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_HP) + 0);
      break;
    case MB_HOLD_PF_CUTOFF_HP_1:
      *value = *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_HP) + 1);
      break;
    case MB_HOLD_PF_MODE:
      *value = conf.pf.mode;
      break;
...
Status_t MbRtu_WriteHoldingRegCallback(uint16_t address, uint16_t value)
{
  ...
  switch (address)
  {
    ...
    case MB_HOLD_COM_MB_TIMEOUT:
      ...
      break;
    case MB_HOLD_PF_CUTOFF_LP_0:
      *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_LP) + 0) = value;
      break;
    case MB_HOLD_PF_CUTOFF_LP_1:
      *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_LP) + 1) = value;
      id = CONF_PF_CUTOFF_LP;
      break;
    case MB_HOLD_PF_CUTOFF_HP_0:
      *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_HP) + 0) = value;
      break;
    case MB_HOLD_PF_CUTOFF_HP_1:
      *((uint16_t *)CONF_PTR(CONF_PF_CUTOFF_HP) + 1) = value;
      id = CONF_PF_CUTOFF_HP;
      break;
    case MB_HOLD_PF_MODE:
      conf.pf.mode = (pf_mode_t)value;
      id = CONF_PF_MODE;
      break;
    case MB_HOLD_PF_RESET:
      conf.pf.reset = value;
